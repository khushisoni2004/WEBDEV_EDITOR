const socket = window.createClassroomSocket();
...
$('startForm').addEventListener('submit', event => { event.preventDefault(); if(!socket.connected){$('startError').textContent='Connecting to live server... Please wait a moment for the free server to wake up (takes 30-50s).';$('startError').style.color='#b45309';return;}$('startError').textContent='';socket.emit('create-room', {teacherName:$('teacherName').value, password:$('teacherPassword').value, classTitle:$('classTitle').value}, res => { if (!res?.ok){$('startError').textContent=res?.message||'Could not create room.';$('startError').style.color='';return;}roomId=res.roomId; $('roomCode').textContent=roomId; setTeacherCode(res.teacherCode); $('startModal').style.display='none'; toast(`${res.classTitle} created`); }); });
