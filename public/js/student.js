...
$('joinForm').onsubmit=e=>{e.preventDefault();$('joinError').textContent='';socket.emit('join-student',{roomId:$('roomInput').value,studentName:$('studentName').value},res=>{if(!res?.ok){$('joinError').textContent=res?.message||'Unable to join classroom.';return;}roomId=res.roomId;$('roomCode').textContent=roomId;$('teacherInfo').textContent='Teacher: '+res.teacherName;$('teacherTitle').textContent=res.teacherName+' — Live Code';const saved=JSON.parse(localStorage.getItem(`codelab:${roomId}`)||'null');setMine(saved||res.myCode);setTeacher(res.teacherCode);if(res.teacherLastRun?.mode==='js')renderCapturedOutput($('wConsole'),res.teacherLastRun.output);$('joinModal').style.display='none';toast('Joined classroom successfully');});};
