...
$('joinForm').onsubmit=e=>{e.preventDefault();$('joinError').textContent='';socket.emit('join-student',{roomId:$('roomInput').value,studentName:$('studentName').value},res=>{if(!res?.ok){$('joinError').textContent=res?.message||'Unable to join classroom.';return;}roomId=res.roomId;$('roomCode').textContent=res.practiceOnly?'PRACTICE':roomId;$('teacherInfo').textContent=res.practiceOnly?'Personal practice — no teacher connected':'Teacher: '+res.teacherName;$('teacherTitle').textContent=res.practiceOnly?'Personal Practice':res.teacherName+' — Live Code';const saved=JSON.parse(localStorage.getItem(`codelab:${roomId}`)||'null');setMine(saved||res.myCode);setTeacher(res.teacherCode);if(res.teacherLastRun?.mode==='js')renderCapturedOutput($('wConsole'),res.teacherLastRun.output);$('joinModal').style.display='none';toast(res.practiceOnly?'Practice workspace ready':'Joined classroom successfully');});};
